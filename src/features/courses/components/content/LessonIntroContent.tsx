  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {lesson.measurableDeliverables && lesson.measurableDeliverables.length > 0 && (
        <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 sm:p-5 animate-slide-up hover-lift">
          ...
          <div className="space-y-2 sm:space-y-3">
            {lesson.measurableDeliverables.map((deliverable: any) => (
              <label 
                ...
                className="flex items-start space-x-2 sm:space-x-3 cursor-pointer hover:bg-blue-100 p-1 sm:p-2 rounded-md transition-colors duration-200"
              >
                <input
                  type="checkbox"
                  checked={completedTasks.includes(deliverable._key)}
                  onChange={(e) => handleTaskCompletion(deliverable._key, e.target.checked)}
                  className="h-4 w-4 sm:h-5 sm:w-5 text-blue-600 rounded border-gray-300 focus:ring-blue-500 focus:ring-offset-2 flex-shrink-0 mt-1 transition-all duration-200"
                  aria-describedby={`task-${deliverable._key}-description`}
                />
                <div 
                  id={`task-${deliverable._key}-description`}
                  className={`text-sm sm:text-base text-gray-700 transition-all duration-200 ${
